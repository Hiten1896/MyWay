...
async function searchWithFallback(query) {
    try {
        ...
        return (result.videos || []).slice(0, 12).map(normalizeVideo);
    } catch (error) {
        console.warn('yt-search failed, trying yt-dlp fallback:', error.message);
        const result = await ytDlp(`ytsearch12:${query}`, {
            dumpSingleJson: true,
            flatPlaylist: true,
            noWarnings: true,
            skipDownload: true
        });
        return (result.entries || []).filter(video => video.id).map(normalizeDlpVideo);
    }
}
